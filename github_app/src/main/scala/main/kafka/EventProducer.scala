...
class EventProducer(val brokers: String) extends java.io.Serializable {
	...
	def sendMessage(topic: String, event: Event) : Unit = {
		val record = new ProducerRecord[String, String](topic, event.key, event.toString)
